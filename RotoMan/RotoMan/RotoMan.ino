...
//--------------------------------------------------------------------------------------------
  ...
  delay(15);                           // change this for a different arduino, (loop) speeds.
...
//-------------------------encoder----------------------------------------------
      signed char pos = encoder.getPosition();
        if (pos != 0) {
          position += pos;
             EEPROM.put(5, position);   // save the encoder value if its powered down.
               encoder.reset();
                 if (position < 1){
                    position = 0;
                 }
...
//------------------------encoder  button----------------------------------------
/*
 * put out on TTL TxRx pins to communicate with the outside world, to set
 * a ctcss tone gererator or RGB string or whatever......
 */
               ...
               if (buttonState == LOW) {
                  buttonPresses++; 
                     switch(buttonPresses){
                        case 1:
                           Serial.println("1017");  // example values
                           EEPROM.put(5, 1017);     // to display
                        break;
                        case 2:
                          Serial.println("5678");
                          EEPROM.put(5, 5678);
